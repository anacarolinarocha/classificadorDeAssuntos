...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 22 18:59:45 2019

@author: anarocha
"""

select_teste="""WITH vs_nivel_assunto as(SELECT b.*,
CASE 
		WHEN b.nivel = '2' THEN 
...
((((((((((to_char(ptrf.nr_sequencia, 'FM0000000'::text) || '-'::text) || to_char(ptrf.nr_digito_verificador, 'FM00'::text)) || '.'::text) || to_char(ptrf.nr_ano, 'FM0000'::text)) || '.'::text) || "left"(ptrf.nr_identificacao_orgao_justica::character varying::text, 1)) || '.'::text) || to_char("right"(ptrf.nr_identificacao_orgao_justica::character varying::text, 2)::integer, 'FM00'::text)) || '.'::text) || to_char(ptrf.nr_origem_processo, 'FM0000'::text)) AS nr_processo,
doc.id_processo_documento, 
right(doc.ds_identificador_unico,7) as codigo_documento,
doc.dt_juntada,
docbin.ds_modelo_documento
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2546' AND pa.in_assunto_principal='S')),0) AS in_2546
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2594' AND pa.in_assunto_principal='S')),0) AS in_2594
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2086' AND pa.in_assunto_principal='S')),0) AS in_2086
...
INNER JOIN tb_processo_trf ptrf ON ptrf.id_processo_trf = doc.id_processo
...
INNER JOIN pje.tb_processo_assunto pa2 ON pa2.id_processo_trf = doc.id_processo AND pa2.in_assunto_principal='S' AND pa2.id_assunto_trf in (SELECT id_assunto_trf FROM tb_assunto_trf where cd_assunto_trf in ('2546','2594','2086') )
WHERE doc.id_processo IN 
		(SELECT id_processo FROM  
			...
			(SELECT id_processo, count(id_processo_documento) as total_recursos_ordinarios 
			 ...
			 AND dt_juntada < '2018-12-31' AND dt_juntada > '2016-01-01'
