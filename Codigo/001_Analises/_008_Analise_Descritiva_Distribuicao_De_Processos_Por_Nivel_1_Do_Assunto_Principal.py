...
SELECT a.*, 
	...
	WHERE in_ativo = 'S' and cd_assunto_trf not in ('12467','12468','12469','12470','12471')
) as b)
select vs.cd_assunto_nivel_1, 
case 
	when cd_assunto_nivel_1 = '864' then 'DIREITO DO TRABALHO'
	when cd_assunto_nivel_1 = '9985' then 'DIREITO ADMINISTRATIVO E OUTRAS MATÉRIAS DE DIREITO PÚBLICO'
	when cd_assunto_nivel_1 = '8826' then 'DIREITO PROCESSUAL CIVIL E DO TRABALHO'
	when cd_assunto_nivel_1 = '899' then 'DIREITO CIVIL'
	when cd_assunto_nivel_1 = '6191' then 'DIREITO INTERNACIONAL'
else 'DESCONHECIDO' end as ds_assunto,
count(*)
from pje.tb_processo_assunto pa 
inner join vs_nivel_assunto vs on pa.id_assunto_trf = vs.id_assunto_trf and pa.in_assunto_principal = 'S'
group by cd_assunto_nivel_1,ds_assunto
"""
...
total = df.groupby(['cd_assunto_nivel_1','ds_assunto']).sum()
total.plot(kind='bar')
plt.show()

total.to_csv('/home/anarocha/myGit/classificadorDeAssuntos/Codigo/001_Analises/Outputs/Total_De_Processos_Por_Nivel_1_Do_Assunto_Principal.csv')
