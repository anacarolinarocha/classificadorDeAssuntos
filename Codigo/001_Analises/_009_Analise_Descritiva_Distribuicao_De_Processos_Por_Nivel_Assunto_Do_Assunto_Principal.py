...
select (SELECT cd_sigla_tribunal from tb_tribunal) AS tribunal, vs.nivel, 
count(*)
...
total = df.groupby(['nivel']).sum()
total.plot(kind='bar')
plt.show()

total.to_csv('/home/anarocha/myGit/classificadorDeAssuntos/Codigo/001_Analises/Outputs/Total_De_Processos_Por_Nivel_Assunto_Do_Assunto_Principal.csv')
