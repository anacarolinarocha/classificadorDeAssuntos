...
ojc.ds_orgao_julgador_colegiado AS identificador_documento,
...
right(doc.ds_identificador_unico,7),
doc.dt_juntada,
docbin.ds_modelo_documento
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2546')),0) AS in_2546
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2594')),0) AS in_2594
,COALESCE((SELECT 1 WHERE EXISTS (SELECT * FROM tb_processo_assunto pa INNER JOIN vs_nivel_assunto va ON pa.id_assunto_trf = va.id_assunto_trf WHERE pa.id_processo_trf = doc.id_processo AND cd_assunto_nivel_3 = '2086')),0) AS in_2086
